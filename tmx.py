...
def hex_to_tuple(s):
    # Converts a string with hex rgb color code to a tuple of decimal ints
    # '#ff1200' --> (255, 18, 0)
    s = s[1:]
    r, g, b = s[:2], s[2:4], s[4:]
...
class Tile(object):
    def __init__(self, gid, tileset):
        ...
        self.animated = False
        self.frames = []  # frame_id -> {surf: Surface, duration: int}
        self.timer = 0
        self.current_id = 0
    ...
    def loadxml(self, tag, images):
        log.debug("Loading xml data for %s" % self.__repr__())
        ...
        anim = tag.find('animation')
        if anim is not None:
            self.animated = True
            for c in anim.findall('frame'):
                tileid = int(c.attrib['tileid'])
                duration = float(c.attrib['duration']) / 1000.
                self.frames.append({'surf': images[tileid], 'duration': duration})
            self.frames = tuple(self.frames)
            self.surface = self.frames[0]['surf']
            log.debug("Tile frames: %s" % repr(self.frames))
        else:
            self.surface = images[self.gid - self.tileset.firstgid]
    ...
    def __repr__(self):
        return '<Tile %d: %s>' % (self.gid, repr(self.properties))
...
class Tileset(object):
    ...
    @classmethod
    def fromxml(cls, tag, firstgid=None, dir_name=''):
        if 'source' in tag.attrib:
            dir_name = os.path.split(tag.attrib['source'])[0]           
            firstgid = int(tag.attrib['firstgid'])
            with open(tag.attrib['source']) as f:
                log.debug("Reading tileset from %s" % tag.attrib['source'])
                tileset = ElementTree.fromstring(f.read())
            return cls.fromxml(tileset, firstgid, dir_name)

        name = tag.attrib['name']
        log.debug("Loading data for tileset '%s'" % name)
        if firstgid is None:
            firstgid = int(tag.attrib['firstgid'])
        ...
        frames = tileset.load_image(os.path.join(dir_name, image_tag.attrib['source']))
        for c in tag.findall('tile'):
            gid = tileset.firstgid + int(c.attrib['id'])
            tile = Tile(gid, tileset)
            tile.loadxml(c, frames)
    ...
    def load_image(self, file):
        ...
        if not image:
            sys.exit("Error loading source image: file %s not found" % file)
        ...
        for line in xrange(image.get_height() / self.tile_height):
            for column in xrange(image.get_width() / self.tile_width):
                ...
                frames[id] = image.subsurface(pos)
                id += 1

        log.debug("Tileset: got %d tiles from %s" % (len(frames), file))
        return frames
...
class Tilesets(dict):
    '''
    Holds a plain collection of tiles from all tilesets in
    tilemap. A key represents a gid of a tile.

    If a tilemap has two tilesets when the first one 
    has two tiles and the second one has one tile
    a resulting Tilesets dict will have the following
    structure:

    Tilesets[0] -> tile1 (tileset1.firstgid = 0)
    Tilesets[1] -> tile2
    Tilesets[2] -> tile3 (tileset2.firstgid = 2)
    '''
    ...
    def update(self, dt, *args):
        ...
        '''
        #  TODO: optimize animated_tiles collection?
        animated_tiles = filter(lambda t: t.animated == True, self.values())
        for tile in animated_tiles:
            duration = tile.frames[tile.current_id]['duration']
            tile.timer += dt

            if tile.timer >= duration:
                tile.current_id = (tile.current_id + 1) % len(tile.frames)
                tile.surface = tile.frames[tile.current_id]['surf']
                tile.timer -= duration

    def find(self, *properties):
        '''Find all tiles with the given properties set.
        '''
        r = []
        for propname in properties:
            for tile in self.values():
                if tile and propname in tile:
                    r.append(tile)
        return r
    ...
    def match(self, **properties):
        ...
        for propname in properties:
            for tile in self.values():
                if propname not in tile:
                    continue
                if properties[propname] == tile[propname]:
                    r.append(tile)
        return r

    def __repr__(self):
        return '<Tilesets: %s>' % super(Tilesets, self).__repr__()
...
class Cell(object):
    ...
    def __init__(self, x, y, px, py, tile):
        ...
        self.rect = pygame.Rect(px, py, tile.tile_width, tile.tile_height)
        self.top = py
...
class Layer(object):
    ...
    def __setitem__(self, pos, tile):
        '''
        Insert a new tile into the Layer

        pos - a tuple of tile coordinates
        tile - a gid of a tile (see Tilesets docstring for more details)
        '''
    ...
    def draw(self, surface):
        ...
        for x in xrange(ox, ox + w + self.tile_width, self.tile_width):
            i = x // self.tile_width
            for y in xrange(oy, oy + h + self.tile_height, self.tile_height):
    ...
    def get_in_region(self, x1, y1, x2, y2):
        ...
        return [self.cells[i, j]
            for i in xrange(int(i1), int(i2))
                for j in xrange(int(j1), int(j2))
    ...
    def remove_cell(self, tile_pos):
        del self.cells[tile_pos]
...
class Object(object):
    ...
    def draw(self, surface, view_x, view_y, border_color=(255, 100, 100)):
        if not self.visible:
            return
    ...
    def contains(self, x, y):
        return self.intersects(x, y, x, y)
...
class ObjectLayer(object):
    ...
    def draw(self, surface):
        ...
        for object in self.objects:
            object.draw(surface, self.view_x, self.view_y, self.color)
...
class SpriteLayer(pygame.sprite.AbstractGroup):
    def __init__(self, ignore_update=False):
        super(SpriteLayer, self).__init__()
        self.visible = True
        self.ignore_update = ignore_update  # it will not by updated from TileMap.update()
...
class TileMap(object):
    ...
    def update(self, dt, *args):
        for layer in self.layers:
            if hasattr(layer, 'ignore_update') and layer.ignore_update: continue
    ...
    def load(cls, filename, viewport):
        ...
        tilemap.bg_color = hex_to_tuple(map.attrib['backgroundcolor'])
